# Timekeeping Entries History

- **Function:** `get_timekeeping_entries_history` (in `src/timekeeping_entries/history/api.py`)
- **HTTP path:** `/timekeeping_entries/history/` (full URL = `${RHUMBIX_BASE_URL}/timekeeping_entries/history/`)
...
| ids | List[int] | Filter by one or more timekeeping entry IDs. |
...
```python
rows = get_timekeeping_entries_history(history_type="UPDATED", history_start_date="2024-01-01")
```
