# Employee Shifts and Breaks History

- **Function:** `get_employee_shifts_and_breaks_history` (in `src/employee_shifts_and_breaks_history/api.py`)
- **HTTP path:** `/employee_shifts_and_breaks/history/` (full URL = `${RHUMBIX_BASE_URL}/employee_shifts_and_breaks/history/`)
...
| ids | List[int] | Filter by one or more shift extra entry IDs. |
...
```python
rows = get_employee_shifts_and_breaks_history(history_type="UPDATED", history_start_date="2024-01-01")
```
