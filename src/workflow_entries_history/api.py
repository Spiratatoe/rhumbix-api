...
WORKFLOW_ENTRIES_HISTORY_ENDPOINT = "/workflow_entries/history/"

def get_workflow_entries_history(
    ...
    """
    Retrieves a list of workflow entry history records from the Rhumbix API.

    Args:
        page_size (Optional[int]): Number of results to return per page.
        ids (Optional[List[int]]): Filter by one or more workflow entry IDs.
    ...
    Returns:
        List[Dict]: A list of workflow entry history dictionaries.
    """
    url = f"{BASE_URL}{WORKFLOW_ENTRIES_HISTORY_ENDPOINT}"
