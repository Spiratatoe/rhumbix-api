...
TIMEKEEPING_ENTRIES_HISTORY_ENDPOINT = "/timekeeping_entries/history/"

def get_timekeeping_entries_history(
    ...
    """
    Retrieves a list of timekeeping entry history records from the Rhumbix API.

    Args:
        page_size (Optional[int]): Number of results to return per page.
        ids (Optional[List[int]]): Filter by one or more timekeeping entry IDs.
    ...
    Returns:
        List[Dict]: A list of timekeeping entry history dictionaries.
    """
    url = f"{BASE_URL}{TIMEKEEPING_ENTRIES_HISTORY_ENDPOINT}"
